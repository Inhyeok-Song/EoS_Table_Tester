...
void nuc_eos_C_testing_engy_temp_err()
{
   ...
   FILE *ener_err1 = fopen( "ENGY2_engy_err1.txt", "w" );
   FILE *ener_err2 = fopen( "ENGY2_engy_err2.txt", "w" );

   FILE *entr_err1 = fopen( "ENGY2_entr_err1.txt", "w" );
   FILE *entr_err2 = fopen( "ENGY2_entr_err2.txt", "w" );

   FILE *prss_err1 = fopen( "ENGY2_prss_err1.txt", "w" );
   FILE *prss_err2 = fopen( "ENGY2_prss_err2.txt", "w" );
   ...
   for ( int i=0; i<6; i++ )
   {
      ...
      {
         fprintf( output_error[i], "#    Num            logrho           logtemp          logenergy          logpress           entropy                Ye         rel_error         abs_error\n", "w" );
      }
   }
  ...
  do {                                                                       \
   ...
   while ( N_data < 927 ) {
      ...
      for ( int i_mode=0; i_mode<3; i_mode++ ) {
         ...
         for ( int j=0; j<iy; j++ ) { //integer_scalars[0]
            for ( int i=0; i<ix; i++ ) {
               real rand1 = 0.04*rand()/(double)RAND_MAX + 0.98;
               real rand2 = 0.04*rand()/(double)RAND_MAX + 0.98;
               real rand3 = 0.04*rand()/(double)RAND_MAX + 0.98;
               real rand4 = 0.04*rand()/(double)RAND_MAX + 0.98;
               real rand5 = 0.04*rand()/(double)RAND_MAX + 0.98;
               real rand6 = 0.04*rand()/(double)RAND_MAX + 0.98;
               ...
               if ( node[i] < 2 ) {
                  ...
                  rel_err = fabs( ( temp - temp0 )/ ( temp0 ) );
                  abs_err = fabs( temp - temp0 );
                  ...
                  else if ( rel_err > 1.e-10 && keyerr == 0 ) { // ) {
                     // error 2: relative errors between the FLASH1D Data and [i_mode] > 0.1%
                     failcount2[i_mode]++;
                     fprintf( output_error[i_mode+3], "%8d    %10.8e    %10.8e    %10.8e    %10.8e    %10.8e    %10.8e    %10.8e    %10.8e    %10.8e\n",
                              failcount2[i_mode], LOG10( dens ), LOG10( temp0 ), LOG10( temp ), LOG10( eps0 + g_energy_shift ), LOG10( eps + g_energy_shift ), LOG10( pres0 ), ye, rel_err, abs_err );
   ...
   fprintf( stdout, "Maximum relative error in energy mode     :  %f\n", max_err[0] );
   fprintf( stdout, "-----------------------------------------------------------------------------\n" );
